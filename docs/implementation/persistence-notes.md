...
## Tables in Phase 4A (indexing manifests)

| Table | Purpose |
|-------|---------|
| `index_manifests` | Snapshot of one indexing build: hashing, provider identity, dims, counts, FK to **`processing_runs`** (`SET NULL` on delete). |
| `index_manifest_chunks` | Per-chunk row (`dense_indexed`, `sparse_indexed`, optional **`sparse_terms_json`**). Phase 4A stores **no** embeddings on **`chunks`**; pgvector deferred to Phase 4B. |

See [`indexing-notes.md`](indexing-notes.md).

## Future work (later phases)

- **Phase 4B**: pgvector column(s), real local embeddings, optional vector indexes.
